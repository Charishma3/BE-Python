...
conn = sqlite3.connect('task14.db')
c = conn.cursor()
c.execute('''CREATE TABLE COFFEEMACHINE3(espresso INT,latte INT,cappucino INT);''')
c.execute('''SELECT * FROM COFFEEMACHINE3''')
records = c.fetchall()
for r in records:
    print("Number of Espresso Bought:", r[0])
    print("Number of Latte Bought:", r[1])
...
class CoffeeMachine(Tk):
    ...
    def main(self):
        def getinput():
            ...
            if (x == "off"):
                top.destroy()
                c.execute('''INSERT INTO COFFEEMACHINE3 VALUES(?,?,?)''', (self.l, self.e, self.c))
                return
            if (x == "report"):
                frame.destroy()
                texti.destroy()
        ...
        E1 = Entry(frame)
        E1.pack()
        b = Button(frame, text="Enter", command=getinput)
        b.pack()
        mainloop()
    ...
    def report(self, value):
        ...
        te.pack()
        te.insert(INSERT,
                  "REPORT\nWater:{}ml\nMilk:{}ml\nCoffee:{}g\nMoney:${}".format(self.water, self.milk, self.coffee,
                                                                                self.money))
    ...
    def pay(self, value, amount):
        ...
        if (value == "espresso"):
            if (amount < 2.50):
                ...
                b1 = Button(top, text="Go to Main Menu",
                            command=lambda: textt.pack_forget() | self.clean(b1) & self.main())
                b1.pack()
            elif (amount > 2.50):
                textt.insert(INSERT, "Here is ${} dollars in change.".format(round(amount - 3.50, 2)))
                b1 = Button(top, text="Make Coffee",
                            command=lambda: textt.pack_forget() | self.clean(b1) & self.makecoffee(value, textt))
                b1.pack()
            else:
    ...
    def coins(self, value):
        ...
        E5 = Entry(f)
        E5.pack()
        b = Button(f, text="Enter", command=getinputs)
        b.pack()

    def deduct_supplies(self, value):
        text = Text(top)
        ...
        if (value == "espresso"):
            if (self.water < 100):
                ...
                b1 = Button(top, text="Go to Main Menu",
                            command=lambda: text.pack_forget() | self.clean(b1) & self.main())
                b1.pack()
            elif (self.milk < 10):
                text.pack()
